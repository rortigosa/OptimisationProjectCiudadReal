function [mat_info,...
  ...
  weakening_linear_model]        =  UserDefinedModel(model,void_region)
...
gamma                           =  @ SwitchLinearNonlinearFunction;
dgamma                          =  @ DiffSwitchLinearNonlinearFunction;

inf_density_penalty             =  1;  % Penalty term for low densities
sup_density_penalty             =  1;  % Penalty term for high densities

viscosity                       =  @ ViscosityStabilisation;     %  Viscosity to stabilise low-medium density elements
dviscosity                      =  @ DiffViscosityStabilisation; %  Viscosity to stabilise low-medium density elements (derivative)
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function gamma = SwitchLinearNonlinearFunction(rho)
    gamma                          =  ones(length(rho),1);
%    gamma                          =  rho;
%     gamma                          =  rho.^2;
%     gamma                          =  rho.^3;
%     gamma                          =  rho.^4;
...
%     elasticity_limit               =  1;
%     gamma                          =  rho;
%     gamma(rho<elasticity_limit)    =  rho(rho<elasticity_limit)/elasticity_limit;
%     gamma(rho>elasticity_limit)    =  1;
%     limit1                           =  0.3;   
%     limit2                           =  0.999;
%     value1                           =  0.0;
%     slope                            =  (1 - value1)/(limit2 - limit1);
%     gamma                            =  rho;
%     gamma(rho<limit1)                =  value1;
%     gamma(rho>limit1 & rho<limit2)   =  value1 + slope*(rho(rho>limit1 & rho<limit2) - limit1);
%     gamma(rho>limit2)                =  1;
end
function dgamma = DiffSwitchLinearNonlinearFunction(rho)
    dgamma                          =  zeros(length(rho),1);
%    dgamma                          =  ones(length(rho),1);
%     dgamma                          =  2*rho.^1;
%     dgamma                          =  3*rho.^2;
%     dgamma                          =  4*rho.^3;
%    dgamma                          =  1/2*rho.^(-1/2);
%     elasticity_limit               =  1;
%     dgamma                         =  rho;
%     dgamma(rho<elasticity_limit)   =  elasticity_limit;
%     dgamma(rho>elasticity_limit)   =  0;
%     limit1                           =  0.3;
%     limit2                           =  0.999;
%     value1                           =  0.0;
%     slope                            =  (1 - value1)/(limit2 - limit1);
%     dgamma                           =  rho;
%     dgamma(rho<limit1)               =  0;
%     dgamma(rho>limit1 & rho<limit2)  =  slope;
%     dgamma(rho>limit2)               =  0;
end
function viscosity = ViscosityStabilisation(rho)
    viscosity                          =  zeros(length(rho),1);
    %    viscosity                          =  1e-1*(1 - rho);
%    viscosity                          =  1e-8*(1 - rho.^3);
%     gamma                          =  rho.^2;
%     gamma                          =  rho.^3;
%     gamma                          =  rho.^4;
%    gamma                          =  rho.^(1/2);
...
function dviscosity = DiffViscosityStabilisation(rho)
%    dviscosity                          =  -1e-1*ones(length(rho),1);
    dviscosity                          =  zeros(length(rho),1);
%    dviscosity                          =  -1e-8*3*rho.^2;
%     dgamma                          =  2*rho.^1;
%     dgamma                          =  3*rho.^2;
%     dgamma                          =  4*rho.^3;
%    dgamma                          =  1/2*rho.^(-1/2);
