function [xPhys,Solution] = Compliance3D(nelx,nely,nelz,volfrac,rmin,dir,x,...
                              load_increment,Optimisation,...
                              Geometry,Data,TimeIntegrator,FEM,Quadrature,...
                              NR,MatInfo,Bc,Solution,Mesh,...
                              Assembly,UserDefinedFuncs,PostProc)
...
c0                     =  1;
xPhys0                 =  xPhys;
while (change > user_change) && (iter < 200)
    ...
    %----------------------------------------------------------------------
    [c,c0,dc,Solution,...
     Optimisation]     =  FEMSolver(xPhys(:),xPhys0(:),iteration,c0,Data,TimeIntegrator,NR,Geometry,Mesh,...
    ...
    xold1              =  xval;   
    %----------------------------------------------------------------------        
    % UPDATE VARIABLES FROM MMA
    %----------------------------------------------------------------------        
    x                  =  reshape(xmma,nely,nelx,nelz); 
    xTilde(:)          =  (H*x(:))./Hs;
    xPhys              =  (tanh(beta*eta) + tanh(beta*(xTilde-eta)))/(tanh(beta*eta) + tanh(beta*(1-eta)));   
    ...
    %----------------------------------------------------------------------            
    subplot(3,2,1)
    plot3(Solution.x.Eulerian_x(1,:),Solution.x.Eulerian_x(2,:),Solution.x.Eulerian_x(3,:),'o','MarkerSize',2)
    hold on
    axis equal    
    %----------------------------------------------------------------------                
    % PLOT DENSITIES  
    %----------------------------------------------------------------------            
    subplot(3,2,2)
    xPhys_  =  reshape(xPhys(1:nelx*nely),nely,nelx);
    colormap(gray); imagesc(1-xPhys_); caxis([0 1]); axis equal; axis off; drawnow;
    %----------------------------------------------------------------------                
    ...
    colormap(gray); imagesc(reshape(Optimisation.stability(1:nelx*nely),size(xPhys_,1),size(xPhys_,2))); caxis([0 1]); axis equal; axis off; drawnow;
    %----------------------------------------------------------------------                
