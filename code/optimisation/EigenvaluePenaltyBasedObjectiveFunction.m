...
function penalty         =  EigenvaluePenaltyBasedObjectiveFunction(Optimisation,Solution,Mesh,MatInfo,Quadrature,FEM)    
...
for ielem=1:Mesh.volume.n_elem  
    ...
    % Penalty term
    %----------------------------------------------------------------------
    %penalty  =  penalty + EigenvaluePenaltyBasedTerm(rho_p(ielem),drho_p(ielem),Elasticity,IntWeight);
    penalty   =  penalty + EigenvaluePenaltyBasedTerm(1,0,Elasticity,IntWeight);
 end     

fprintf('Penalty value = %f',penalty)
