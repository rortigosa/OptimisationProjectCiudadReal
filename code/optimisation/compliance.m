...
function xPhys         =  compliance(nelx,nely,volfrac,rmin,str,x,load_increment)
...
%--------------------------------------------------------------------------
%E0                    =  1;
%E0                     =  InitialComplianceScaling(str,xPhys);
%E0                    =  1e-1;
iteration              =  0;   % 
user_change           =  0.005;
%user_change            =  0.01;
...
Penalty                =  zeros(1000,1);
c0                     =  1;
penalty0               =  1;

while (change > user_change)  && (loop < 350) || (instability_flag == 1)
%while (change > 0.001) && (loop < 1000)
    ...
    xold               =  x;    
    %----------------------------------------------------------------------
    % FE-ANALYSIS (output:derivative of objective function and displacements)
    %----------------------------------------------------------------------
   str.mat_info.optimisation.void_factor_scaling  =  1e0;     
    [c,c0,penalty,penalty0,dc,dpenalty,u,... 
   instability_flag,str]   =  FEMSolver(str,xPhys(:),iteration,c0,penalty0);

    dc                =  dc + dpenalty;  
    %dc                 =  (dc + dpenalty)/(c0 + penalty0);  
    %----------------------------------------------------------------------
    % OBJECTIVE FUNCTION, CONSTRAINTS AND SENSITIVITY ANALYSIS
    %----------------------------------------------------------------------
    %ce                =  reshape(sum((U(edofMat)*KE).*U(edofMat),2),nely,nelx);
    %c                 =  sum(sum((Emin+xPhys.^penal*(E0-Emin)).*ce));
    %c                  =  (load_factor*str.bc.Neumann.force_vector'*u)/E0 + inst_penalty;    
    %c                 =  (str.bc.Neumann.force_vector'*u)/E0 + inst_penalty;    
    c                  =  c + penalty;
    %c                  =  (c + penalty)/(c0 + penalty0);
    ObjFunc(iteration) =  c;
    ...
    df0dx              =  dc;    
    [xmma,~,~,~,~,...
     ~,~,~,~,low,upp]  =  mmasub(m,n_val,loop,xval(:),xmin,xmax,xold1(:),xold2(:), ...
    ...
    fprintf('-----------------------------------------------\n')
    fprintf('Load Number:%11.4f\n',load_increment);    
    if strcmp(str.data.optimisation.type,'nodal_force_location_compliance')
       fprintf(' It.:%5i Obj.:%11.4f Displ.:%11.4f Vol.:%7.3f ch.:%7.3f\n',loop,c,0,mean(xPhys(:)),change);    
    elseif strcmp(str.data.optimisation.type,'discrete_displacement')

    elseif strcmp(str.data.optimisation.type,'global_displacement')
    ...
    elseif strcmp(str.data.optimisation.type,'energy')
       fprintf(' It.:%5i Obj.:%11.4f Max Horz. Displ.:%11.4f Max Vert. Displ.:%11.4f Vol.:%7.3f ch.:%7.3f\n',loop,c,...
                                                          max(abs(u(1,:))),max(abs(u(2,:))),mean(xPhys(:)),change);    
    end
    %----------------------------------------------------------------------                
    ...
    %figure(2)
    subplot(4,2,4)
    plot(1:iteration,ObjFunc(1:iteration),'-o','LineWidth',4)
    
    subplot(4,2,5)
    if ~isfield(str,'optimisation')
       str.optimisation.stability = ones(size(xPhys,1),size(xPhys,2));
    end
    colormap(gray); imagesc(reshape(str.optimisation.stability,size(xPhys,1),size(xPhys,2))); caxis([0 1]); axis equal; axis off; drawnow;

    subplot(4,2,7)
    plot(1:iteration,Penalty(1:iteration),'-o','LineWidth',4)
...
%    if beta < 3000 && (loopbeta >= 50 || change <= user_change)
        loopbeta = 0;
        change   = 1;
        beta     = 2*beta;
