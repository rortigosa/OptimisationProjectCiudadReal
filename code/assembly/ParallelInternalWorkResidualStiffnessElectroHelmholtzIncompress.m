function asmb      =  ParallelInternalWorkResidualStiffnessElectroHelmholtzIncompress(ielem,quadrature,solution,geometry,mesh,fem,...
...
%--------------------------------------------------------------------------
asmb               =  ElementResidualMatricesInitialisationElectroIncompressible(geometry,mesh);
%--------------------------------------------------------------------------
% Obtain gradients of kinematics and electrical variables
%--------------------------------------------------------------------------
[kinematics,...
    DN_X_x]        =  KinematicsFunctionVolume(geometry.dim,...
...
for igauss=1:ngauss
    ...
    %----------------------------------------------------------------------
    asmb.Tx        =  asmb.Tx   +  ...
                                 (BF(:,:,igauss)'*Piola_vectorised(:,igauss))*Int_weight;
    %----------------------------------------------------------------------
...
for igauss=1:ngauss
    ...
    %----------------------------------------------------------------------
    vect_mat       =  VectorisedStiffnessMatricesElectroHelmholtzIncompressible(igauss,BF(:,:,igauss),DN_X_phi(:,:,igauss),...
                                                           mat_info.derivatives.D2Psi,...
                                                           mat_info.derivatives.DPsi,...
