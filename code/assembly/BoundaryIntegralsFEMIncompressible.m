...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function str              =  BoundaryIntegralsFEMIncompressible(str)
...
for iedge=1:size(str.mesh.surface.x.boundary_edges,2)
    ...
    %----------------------------------------------------------------------
    edge_assembly         =  ResidualStiffnessElectroBoundaryFEM(iedge,str.geometry.dim,str.quadrature,str.fem,...
                                               str.mesh,str.solution,str.vectorisation,str.material_information);    
    %----------------------------------------------------------------------
    % Sparse assembly
    %----------------------------------------------------------------------
    [INDEXI,INDEXJ,DATA]  =  StiffnessSparseAssemblyBoundaryFEMIncompressible(iedge,str.geometry.dim,str.mesh,edge_assembly);
    ...
    %----------------------------------------------------------------------
    [INDEXI,INDEXJ,DATA]  =  VectorsAssemblyBoundaryFEM(iedge,str.geometry.dim,str.mesh,edge_assembly);
