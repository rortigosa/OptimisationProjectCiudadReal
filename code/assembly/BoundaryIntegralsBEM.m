...
for inode=1:str.mesh.surface.q.n_nodes
    for iedge=1:size(str.mesh.surface.x.boundary_edges,2)
        ...
        edge_assembly          =  ResidualStiffnessElectroBoundaryBEM(inode,iedge,str.geometry.dim,...
                                                                    str.mesh,str.fem,str.solution,str.quadrature,str.vectorisation);
        %------------------------------------------------------------------
        % Sparse assembly
        %------------------------------------------------------------------
        [INDEXI,INDEXJ,DATA]   =  StiffnessSparseAssemblyBoundaryBEM(inode,str.geometry.dim,iedge,str.mesh,edge_assembly);                                                                      
        ...
        %------------------------------------------------------------------
        [INDEXI,INDEXJ,DATA]   =  SparseVectorsAssemblyBoundaryBEM(str.geometry.dim,inode,str.mesh,edge_assembly);
