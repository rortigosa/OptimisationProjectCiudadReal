...
void perfomer(mwSize dim,
        ...
        const double *ElasticityOrigin,
        double *stability)
{
    /*-------------------------------------------------------------------*/
    /*Temporaries within the C function*/
    /*-------------------------------------------------------------------*/
    double *Temp_           = (double*)malloc(sizeof(double)*dim*dim*dim*dim);    
    double *minimum_vector  = (double*)malloc(sizeof(double)*dim*dim);
    double *minimum_gauss   = (double*)malloc(sizeof(double)*ngauss);
    /*-------------------------------------------------------------------*/
    /*The C function*/
    /*-------------------------------------------------------------------*/
    double zero_;    
    ...
    /*-------------------------------------------------------------------*/
    absolute_minimum      =  sequential_minimum_vector(minimum_gauss,ngauss);    
    stability[0]          =  1.0;
    AbsVector(&ElasticityOrigin[0], (size_t)total_dim, Temp_);       
    zero_                 =  sequential_maximum_vector(Temp_, total_dim);    
    zero_                 =  AbsDouble(zero_*1e-6)*(-1);
    if (absolute_minimum<=zero_){
       stability[0]       =  0.0;
    }
    /*-------------------------------------------------------------------*/
    /* Free temporaries*/
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    ElasticityOrigin   =  mxGetPr(prhs[3]);   
    /*-------------------------------------------------------------------*/
    ...
    /*-------------------------------------------------------------------*/
    perfomer((mwSize)dim,(mwSize)ngauss, Elasticity, ElasticityOrigin, stability);
}
