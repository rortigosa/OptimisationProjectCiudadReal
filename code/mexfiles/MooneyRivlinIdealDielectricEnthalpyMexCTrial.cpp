...
//     double E0_E0_invariant           =  trace(E0_E0);
    ...
    /*-------------------------------------------------------------------*/
     ...
     //Tensor<double,ndim,ndim> WF  =  mu1*F - epsilon/J*(E0_HE0_invariant*Imatrix - E0_HE0);
...
//         
    ...
    Tensor<double,ndim,ndim,ndim,ndim> Elasticity4D  =  WFF + DH_WHH_DH + WJJ*H_H + C_Geom;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    plhs[0]              =  mxCreateNumericArray(3, ElastDims, mxDOUBLE_CLASS, mxREAL);   /*P*/
    ...
    if (dim==2){
       for (igauss=0; igauss<ngauss; ++igauss){
           MooneyRivlin<2>(mu1,mu2,lambda,epsilon,&F[dim*dim*igauss],&H[dim*dim*igauss],J[igauss],&Elasticity[dim*dim*dim*dim*igauss]);
       }
    }
    else if (dim==3){
       for (igauss=0; igauss<ngauss; ++igauss){
           MooneyRivlin<3>(mu1,mu2,lambda,epsilon,&F[dim*dim*igauss],&H[dim*dim*igauss],J[igauss],&Elasticity[dim*dim*dim*dim*igauss]);
       }    
    }
}
