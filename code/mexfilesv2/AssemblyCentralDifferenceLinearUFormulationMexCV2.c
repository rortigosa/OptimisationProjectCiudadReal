...
void perfomer(mwSize dim,
        ...
        const double *FixDofs,
        const double *NFixedDofs,
        double *x_)
{
    /*-------------------------------------------------------------------*/
    /*Temporaries*/
    ...
    double *TInternal    = (double*)malloc(sizeof(double)*(dim*n_nodes));
    /*-------------------------------------------------------------------*/
...
//         /*---------------------------------------------------------------*/
    ...
    /*-------------------------------------------------------------------*/
    /* Mass and Damping terms*/
    /*-------------------------------------------------------------------*/
    for (mwSize idof=0; idof<NFixedDofs; i++){
        fixed_dof     =  (mwSize)FixedDofs[idof];
        Residual      =  TInternal[fixed_dof] + Mass[fixed_dof]*acceleration[i] + Damping[fixed_dof]*velocity[fixed_dof] - ForceVector[fixed_dof];   
        x_[fixed_dof] =  X[fixed_dof] - InvMass[fixed_dof]*Residual;
    }
    /*-------------------------------------------------------------------*/
    /* Free temporaries*/
    /*-------------------------------------------------------------------*/
