...
HE0_E0_vectorised      =  Matrix2Vector(dim^2,ngauss,HE0_E0);
...
mat_info.derivatives.D2Psi.D2PsiDHDH    =  mu2*Itensor - e1*MatrixScalarMultiplication(dim^2,dim^2,ngauss,E0matrix_E0matrix1,invJ);
mat_info.derivatives.D2Psi.D2PsiDHDJ    =  e1*reshape(VectorScalarMultiplication(dim^2,ngauss,HE0_E0_vectorised,invJ2),dim^2,1,ngauss);
mat_info.derivatives.D2Psi.D2PsiDHDE0   =  -e1*MatrixScalarMultiplication(dim^2,dim,ngauss,D_HE0HE0_DH_DE0,invJ);

mat_info.derivatives.D2Psi.D2PsiDE0DH   =  permute(mat_info.derivatives.D2Psi.D2PsiDHDE0,[2  1 3]);
mat_info.derivatives.D2Psi.D2PsiDE0DJ   =  reshape(e1*VectorScalarMultiplication(dim,ngauss,HTHE0,invJ2),dim,1,ngauss);
mat_info.derivatives.D2Psi.D2PsiDE0DE0  =  -e1*MatrixScalarMultiplication(dim,dim,ngauss,HTH,invJ);

mat_info.derivatives.D2Psi.D2PsiDJDH    =  reshape(mat_info.derivatives.D2Psi.D2PsiDHDJ,1,dim^2,ngauss);
mat_info.derivatives.D2Psi.D2PsiDJDJ    =  (mu1 + 2*mu2)*invJ2 + kappa - e1*(invJ3.*HTE0_HTE0) + twoD_Jterm;
